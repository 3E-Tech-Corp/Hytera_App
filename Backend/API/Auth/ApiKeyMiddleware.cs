...
public class ApiKeyAuthorizeAttribute : Attribute, IAsyncAuthorizationFilter
{
    ...
    /// </summary>
    /// <param name="requiredScope">The scope required (e.g., "inventory:read")</param>
    ...
    public async Task OnAuthorizationAsync(AuthorizationFilterContext context)
    {
        ...
        if (key == null)
        {
            ...
            context.Result = new UnauthorizedObjectResult(
                ApiResponse.Fail("Invalid or expired API key."));
            return;
        }
        ...
        if (key.AllowedIPsList.Count > 0)
        {
            ...
            {
                logger.LogWarning("API key {PartnerKey} used from unauthorized IP: {IP}",
                    key.PartnerKey, clientIp);
                context.Result = new ObjectResult(
                    ApiResponse.Fail("Access denied from this IP address."))
                {
                    StatusCode = 403
                };
                return;
        ...
        {
            logger.LogWarning("API key {PartnerKey} lacks required scope: {Scope}",
                key.PartnerKey, _requiredScope);
            context.Result = new ObjectResult(
                ApiResponse.Fail($"This API key does not have the required scope: {_requiredScope}"))
            {
                StatusCode = 403
            };
            return;
